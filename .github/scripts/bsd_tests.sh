...
env GOOS=freebsd GOARCH=amd64 go build -gcflags="github.com/bnema/purego/internal/fakecgo=-std" -v ./...
env GOOS=freebsd GOARCH=arm64 go build -gcflags="github.com/bnema/purego/internal/fakecgo=-std" -v ./...
...
echo 'import (' >> main.go
echo '  _ "github.com/bnema/purego"' >> main.go
echo ')' >> main.go
echo 'func main() {}' >> main.go
go mod edit -replace github.com/bnema/purego=$GITHUB_WORKSPACE
go mod tidy
go mod vendor
go build -v .
...
env CGO_ENABLED=0 go test -gcflags="github.com/bnema/purego/internal/fakecgo=-std" -shuffle=on -v -count=10 ./...
